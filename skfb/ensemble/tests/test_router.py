...
class TestRoutingClassifierBasics:
    """Tests basic functionality of `RoutingClassifier`."""
    ...
    def test_fit(self, sample_data, estimators, router):
        ...
        X, y = sample_data
        clf = RoutingClassifier(estimators, router, cv=3)
    ...
    def test_predict(self, sample_data, estimators, router):
        ...
        X, y = sample_data
        clf = RoutingClassifier(estimators, router, cv=2).fit(X, y)
...
class TestRoutingClassifierCosts:
    """Tests cost handling in `RoutingClassifier`."""
    ...
    def test_costs_scalar_broadcasts(self, sample_data, estimators, router):
        ...
        X, y = sample_data
        clf = RoutingClassifier(estimators, router, costs=0.5, cv=2).fit(X, y)
    ...
    def test_costs_array(self, sample_data, estimators, router):
        ...
        X, y = sample_data
        costs = [0.2, 0.8]
        clf = RoutingClassifier(estimators, router, costs=costs, cv=2)
        clf.fit(X, y)
    ...
    def test_costs_affect_routing(self, estimators, router):
        ...
        clf1 = RoutingClassifier(estimators, router, costs=[0.1, 1.0], cv=2)
        clf1.fit(X, y)

        clf2 = RoutingClassifier(estimators, router, costs=[1.0, 0.1], cv=2)
        clf2.fit(X, y)

        # Routing targets should differ when costs differ.
        targets1 = clf1._make_router_targets(X, y)
        targets2 = clf2._make_router_targets(X, y)
...
class TestRoutingClassifierCollectTargetLabels:
    """Tests the `_collect_target_labels` method."""
    ...
    def test_collect_target_labels_correct_predictions(self, estimators, router):
        """Tests `_collect_target_labels` with all correct predictions."""
        y_true = np.array([0, 1, 0])

        clf = RoutingClassifier(estimators, router, costs=[1.0, 2.0])
    ...
    def test_collect_target_labels_picks_lowest_cost(self, estimators, router):
        """Tests that `_collect_target_labels` picks lowest cost among correct."""
        y_true = np.array([0, 1])

        clf = RoutingClassifier(estimators, router, costs=[0.5, 1.0])
    ...
    def test_collect_target_labels_prefers_lower_cost_with_equal_loss(self, estimators, router):
        ...
        clf = RoutingClassifier(estimators, router, costs=[0.5, 2.0])
        clf.costs_ = np.array([0.5, 2.0])
